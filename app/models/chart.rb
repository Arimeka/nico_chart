...
class Chart < ActiveRecord::Base
  ...
  def self.get
    ...
    page_iteration(@favorites, client)
    page_iteration(@views, client)
    page_iteration(@comments, client)
    page_iteration(@mylist, client)
    delete_old
  end

  def self.page_iteration(page, client)      
    1.upto(100) do |x|        
  ...
  def self.delete_old
    find(:all, :conditions => ["updated_at < ?", 2.days.ago])
  end
end
